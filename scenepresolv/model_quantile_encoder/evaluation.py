...
def quantile_coverage(pred, target):
    ...
    high_coverage = (target < pred[:, 1][:, None]).float().mean(dim=(0, 1))
...
def quantile_mae(pred, target, quantiles=[0.05, 0.95]):
    ...
    mae_low, mae_high = [m.item() for m in maes]

    return mae_low, mae_high
...
def evaluation(dataloader, model, wl, device, epoch, quantiles=[0.05, 0.95]):
    all_pred = []
    all_target = []
    similarities = []
    emb_norms = []
    ...
    mae_low, mae_high = quantile_mae(pred, target, quantiles)
